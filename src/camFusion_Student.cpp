...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    {
        vector<int> count(currFrame.boundingBoxes.size(), 0); 
        int bestmatchId = 0;
        
        for(auto match: matches)
        {
            currKeypoint = currFrame.keypoints[match.trainIdx];
            prevKeypoint = prevFrame.keypoints[match.queryIdx];
            if(prevFrame.boundingBoxes[i].roi.contains(prevKeypoint.pt))
            {
                for(int j=0; j<currFrame.boundingBoxes.size(); j++)
                {
                    if(currFrame.boundingBoxes[j].roi.contains(currKeypoint.pt))
                    {
                        count[j]++;
                    }
                }
        ...
        bestmatchId = std::max_element(count.begin(), count.end()) - count.begin();
        bbBestMatches.insert(std::make_pair(i, bestmatchId));
        cout << "matching bounding box: " << i <<"," << bestmatchId << endl;
    }
